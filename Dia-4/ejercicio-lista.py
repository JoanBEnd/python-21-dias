...
def find_famous_cat(cats):
    ...
    total_max_followers =  max(list(map(lambda x: sum(x["followers"]), cats)))
    return list((elements["name"] for elements in cats if sum(elements["followers"]) == total_max_followers))
    

 

find_famous_cat(lista_gatos)
find_famous_cat(segunda_lista_gatos_)
